...
namespace BlackJack
{
    public class Dealer 
    {
        ...
        public List<Card> dealerCard = new List<Card>();
        ...
        {
            foreach (var CardDeck in card.cards)
            {
                dealerCard.Add(CardDeck);
            }
        }
        ...
        public void DealOutCard(Player player, Dealer dealer, Card card)
        {
            ...
            while (!Winner(player,  dealer))
            {
                ...
                if (userinput == "1" && player.Score < 21)
                {
                    player.hit(card, userinput);
                }
        ...
        {
            return player.Score == 21 || player.Score > 21 || dealer.Score == 21 || dealer.Score >= 17 || dealer.Score > 17 && dealer.Score < player.Score ;
